...
- 初始化新项目后，必须替换模板里的默认密钥；默认管理员用户名和密码以配置文件为准。
- 不要通过 `WEBAPP_ADMIN_PASSWORD` 这类环境变量覆盖管理员密码，否则初始化后的登录口径会和配置文件漂移。
...
- 如果项目会长期运行，JWT、数据库 DSN 等敏感配置可迁移到 `.env`、密钥管理服务、K8s Secret 或其他外部注入方式；管理员账号密码默认仍走配置文件，避免环境变量静默覆盖导致登录失败。
