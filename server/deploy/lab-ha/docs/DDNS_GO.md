...
- 当前直接维护：`lab.saurick.me`
- 当前依赖它的入口：`portal.saurick.me`、`observer.saurick.me`、`ddns.saurick.me` 等当前显式接入 `lab-edge` 公网网关、并采用 `CNAME -> lab.saurick.me` 的子域名；不是所有 `*.saurick.me` 域名都会自动跟随它

## 当前入口

- 内网直连：`http://192.168.0.9:9876`
- 公网登录页：`https://ddns.saurick.me`

说明：

- 当前 `ddns-go` 运行在 `lab-edge / 192.168.0.9`
- Portal 内网模式会直连 `192.168.0.9:9876`
- 公网访问统一通过 `lab-edge` 上的 `Caddy` 反代到 `ddns.saurick.me`
...
- 当前更新频率：`300s`
- 当前直接维护的域名：`lab.saurick.me`
- 当前只同步 `AAAA`：live 配置里 `ipv4.enable=false`、`ipv6.enable=true`，因此这条公网链路当前应按“IPv6 主路径”理解，而不是期待 `ddns-go` 继续维护公网 `A` 记录
- 当前依赖的解析链：当前显式接入 `lab-edge` 公网网关的这组子域，通过 `CNAME -> lab.saurick.me` 跟随它；是否属于这组域名，统一以 `manifests/lab-public-caddy.Caddyfile` 中是否声明该 host 为准
...
curl -6 --noproxy '*' -I https://ddns.saurick.me/login
dig +short AAAA lab.saurick.me @1.1.1.1
```

说明：

- 若当前客户端网络本身没有可用 IPv6，优先改到 `lab-edge` 本机或任一具备 IPv6 的外部网络上验证；不要把“本机出不了 IPv6”误判成 `ddns-go` 没更新
- 当前更可信的外部 DNS 验证方式是 `DoH + AAAA`，例如 `https://dns.google/resolve?name=lab.saurick.me&type=AAAA`
