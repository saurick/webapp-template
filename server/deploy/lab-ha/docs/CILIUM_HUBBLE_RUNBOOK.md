...
- `Hubble` 是否还能看到网络流量
- 当前故障更像 `Cilium datapath`、`Service/Endpoint`，还是外部路由 / 客户端到节点的网络路径
...
for ip in 192.168.0.7 192.168.0.108 192.168.0.128; do
  printf '== %s ==\n' "$ip"
  curl --noproxy '*' -fsS "http://$ip:32668/readyz"
  curl --noproxy '*' -fsS "http://$ip:30089/readyz"
  printf '\n'
done
```
...
- 三台节点的 `32668` 和 `30089` 都通，说明业务正式入口 datapath 基本正常
- 只有单个节点不通，更像节点网络位置或 hostNetwork listener 调度问题，不像 `eBPF` 坏了
...
kubectl --kubeconfig /Users/simon/.kube/ha-lab.conf -n kube-system \
  ...
  egrep '0.0.0.0:32668|0.0.0.0:30089|0.0.0.0:30091|30085|30090'
```

判断：

- 能看到 `0.0.0.0:32668/TCP [HostPort]`、`0.0.0.0:30089/TCP [HostPort]` 或同等条目，说明正式入口已进 Cilium LB map
...
- 在一个终端里跑 `monitor`
- 在另一个终端里重放失败请求，例如 `curl http://192.168.0.108:30089/readyz`
...
- 三台节点的正式入口端口正常，但只有某个客户端失败
- 只有跨 VPN / 跨子网客户端失败，同网段客户端正常
- `cilium-dbg bpf lb list` 里已有正式入口条目，但客户端还是不通
...
- `eBPF` 也在承接当前业务正式入口的 hostNetwork listener
- `BGP` 没开
