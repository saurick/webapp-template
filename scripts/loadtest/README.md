...
- `run.sh` 优先使用本机 `k6`
- 本机未安装 `k6`、但存在 `curl + tar` 时，优先下载固定版本 `k6` 二进制到本地缓存目录；下载默认带超时，避免 shell runner 长时间卡死
- 本机未安装 `k6`、也无法下载二进制，但存在 `go` 工具链时，自动 fallback 到 `go install go.k6.io/k6@v0.49.0`
- 如果当前环境既没有可用 `k6`，也没有 `go/docker`，则 `health/system` 会自动退化到仓库内置的 `curl` fallback，继续产出 `summary.json + report.html`
- 本机既没有本机 `k6`、也没有可用下载/`go` 兜底时，才 fallback 到 `docker run grafana/k6`
...
- `curl fallback` 当前只覆盖最常用的 `health/system`，`auth/mixed` 仍建议在具备 `k6` 的环境执行
