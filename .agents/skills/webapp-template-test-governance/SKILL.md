...
| Template Init | 项目名/服务名替换、模板残留、派生项目收口 | `bash scripts/init-project.sh`；派生项目收口运行 `bash scripts/init-project.sh --project --strict` |
...
| Deploy Preflight | Compose、runtime env、生产配置 | `bash scripts/deploy/production-preflight.sh --env-file server/deploy/compose/prod/.env --runtime`，再做 `/healthz`、`/readyz` |
| Helm / Kustomize / lab-ha Render | chart、values、overlay、Argo 或集群前渲染 | `helm lint`、`helm template`、`kubectl kustomize`，适用时再做 `kubectl ... apply --dry-run=server` |
| Load Test | 性能/容量专项，不是普通功能测试 | `bash scripts/loadtest/run.sh <scenario>`，结果进 `server/deploy/lab-ha/artifacts/loadtest/<run-id>/` |

## Selection Rules

- 模板初始化或项目名替换改动必须检查 `docs/project-init.md`、脚本、README、路径/服务名残留，并实际运行适用的 init 模式与基础 QA。
- health/ready、部署、Compose 或 migration 改动必须覆盖 server 测试、migration 状态和运行时 preflight。
- Helm、Kustomize、lab-ha 或 Argo 变更必须按 `docs/deployment-conventions.md` 选择唯一真源并做 lint/template/kustomize；有可访问集群且目标明确时再做 server-side dry-run，未执行要说明。
