...
- 验证：`helm lint server/deploy/lab-ha/charts/webapp-template -f ...values-prod-trial.yaml -f ...values-prod-trial-internal.yaml` 通过；`bash -n /Users/simon/projects/webapp-template/server/deploy/lab-ha/scripts/check-webapp-prod-trial-bluegreen.sh` 通过；`ruby -rpsych -rjson` 已确认 `/Users/simon/projects/webapp-template/server/deploy/lab-ha/manifests/grafana-lab-gitops-dashboard.yaml` 内嵌 dashboard JSON 合法；`helm template ...values-prod-trial.yaml -f ...values-prod-trial-internal.yaml | kubectl --kubeconfig /Users/simon/.kube/ha-lab.conf apply --dry-run=server -f -` 已确认 `AnalysisTemplate`、`Rollout`、preview `Service` 与相关对象均能被集群 schema 接受；`SKIP_REPO_UPDATE=1 ONLY=lab-platform bash /Users/simon/projects/webapp-template/server/deploy/lab-ha/scripts/helm-release.sh template` 后，对 `lab-platform.yaml` 的 `kubectl --dry-run=server` 也已通过。当前仅完成仓库与 dry-run 校验，尚未实际同步 `prod-trial` live 集群。
- 下一步：如果要把这套蓝绿链路真正上线到实验环境，下一步应在 `argocd` 中手动 sync `webapp-template-prod-trial`，随后用 `check-webapp-prod-trial-bluegreen.sh`、`HA Lab / GitOps & Delivery` 看板和 tracing 脚本做一轮真实 preview -> promote -> post-check 验证。
- 阻塞/风险：当前自动发布验证主要依赖 active/preview `readyz` 与 blackbox 探测，仍不是业务级 SLI；如果后续要把这套蓝绿继续升级成更强自动化门禁，仍建议再补更稳定的业务指标或关键接口 smoke 信号。live 验证还额外暴露了一个第三方 chart 细节：`blackbox-values.yaml` 里的 `serviceMonitor.defaults.labels` 不会自动落到新 target 生成的 `ServiceMonitor.metadata.labels`，因此 `prod-trial active/preview` 这两个 target 需要显式补 `release: kube-prometheus-stack` 才能被当前 Prometheus selector 收进来。仓库里当前还存在若干本轮之外的未提交现场改动与新文件，这次没有回滚它们，只在现有基础上补了蓝绿发布相关变更。
...
- 下一步：若要把值班体验再往前推一层，建议下一轮直接给 `HA Lab / Service Governance` 看板补一个“当前活跃治理告警”面板，并把 `alert-webhook-receiver` 从纯 `echoserver` 升级成能展示最近告警 payload 的轻量接收页，避免现在只能从容器访问日志间接确认。
- 阻塞/风险：这轮演练暴露了一个现场事实：源码里虽然早已加了 Ingress 限流注解，但 live `webapp` Ingress 当时并没有同步到这组配置，说明当前 `Argo/live` 与本地真源仍存在漂移窗口；在完全回收 GitOps 漂移之前，继续做此类治理演练仍需先检查 live 资源实际内容，不能只看仓库 values。另一个盲区是 `alert-webhook-receiver` 目前只是 `echoserver`，没有结构化告警历史页，所以这轮是通过容器访问日志确认 webhook 命中，而不是通过专门 UI 回看 payload。
