...
- 下一步：先把这一轮改动提交并推送到 `origin` / `gitlab`，让 `webapp-template-lab` 项目真正加载新的 `loadtest_lab` job；推送后立刻在 `192.168.0.108:8929` 触发一轮默认 `system` 压测，确认 Portal 动态卡能切到真实的 `通过/失败` 状态并给出 `Open pipeline / Open report`；之后再补 Prometheus remote write 或其他接收端，把压测结果从 GitLab artifacts 继续沉淀到 `192.168.0.108:30081` 的 Grafana `Load Test` dashboard。
- 阻塞/风险：当前“一键压测”仍然依赖 GitLab 页面上的 `Run pipeline` 按钮，Portal 只是预填变量的跳转入口和结果聚合层，不是直接执行器；更关键的是，这批改动目前只在本地工作区，尚未推到远端仓库，所以 `192.168.0.108:8929` 上实际执行的还是旧 CI 配置，`loadtest_lab` 还不会真正出现在 live pipeline 里；另外现有 Prometheus 还没开 remote write receiver，所以压测结果目前先收敛在 GitLab artifacts，而不是 Grafana 实时看板。
