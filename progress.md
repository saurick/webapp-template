...
## 2026-07-07 本地 Vite HMR IPv4 固定

- 完成：排查 `/Users/simon/projects` 下同类 Vite dev runtime 风险后，将模板 `web/vite.config.mjs` 的 HMR 目标固定为 `127.0.0.1:5177`；HTTP 监听继续保留 `0.0.0.0`，后端 proxy 默认已是 `127.0.0.1:8200` 未改。
- 下一步：后续派生项目如保留模板 Vite 配置，可直接继承该 HMR IPv4 口径；若派生项目改端口，需同步改 `clientPort`。
- 阻塞/风险：追加前 `progress.md` 为 352 行、57685 字节，未达到 600 行或 80KB 归档阈值。本轮只改本地开发 Vite 配置，不改模板初始化脚本、部署、schema、RBAC 或正式文档目录。
