...
- 下一步：继续缩小“为什么全节点同时冷启动后 Longhorn 仍会批量 `faulted`”的范围，优先核查 `revisionCounterDisabled=true`、旧 `Unknown/Succeeded` workload 状态残留、以及当前磁盘压力对 replica 选举/复用的影响；目标仍然是不靠人工 salvage 也能收口。
- 阻塞/风险：截至本轮，顺序重启和同时重启都已真实跑通并最终恢复，但“全节点同时断电再上电”仍不是零人工收敛；也就是说，当前环境已经比之前更接近“可上生产的虚拟机级 HA”，但还不能把“同时冷启动无需人工介入”当成已解决事实。
