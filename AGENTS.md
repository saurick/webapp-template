...
- 若本次仅讨论或无文件改动，可跳过更新。
- `progress.md` 只作为过程流水和交接线索，不作为当前模板真源。
...
- 项目专属 skill 只写 webapp-template 的当前真源、边界、命令和验证要求，不把派生项目私有事实、临时任务状态、聊天结论或一次性方案写成模板长期规则。
...
- 若本轮调整了目录结构但判断无需更新 `README / docs`，最终回复和 `progress.md` 必须明确说明未更新的原因与边界。
